...
We show here that $M$ defines the class of Kripke frames in which each world has a successor that does not have strictly more than one successor itself. We'll note this property $N$.

First, we have that $N$ implies $M$. Indeed let us take $\M$ a Kripke model in which $N$ is true. Let $x$ be a world of $\M$. Write $y$ the successor of $x$ that does not have more than one successor.
If $y$ has zero successor, then $\M,x\not\models\Box\Diamond p$ and then $\M,x\models\Box\Diamond p\rightarrow\Diamond\Box p$. If $y$ has exactly one successor $z$ and $\M,x\models\Box\Diamond p$, then $\M,z\models p$ and then $\M,x\models\Box\Diamond p\rightarrow\Diamond\Box p$.

Then, let us show that $M$ implies $N$. Let us take $F$ a Kripke frame in which $M$ is true. Assume that $N$ isn't true, which means that there is one world $x$, such that each of its successors $y_i$ has more than 2 successors. Let us take the valuation $V$ in which $p$ is true in only of the successors of each $y_i$, in such a way that $\forall i,(F,V),y_i\models\Diamond p$. We have that $(F,V),x\models\Box\Diamond p$. However
