...
\end{tikzpicture}
\caption{Exercise 1 question 2: Smallest Bisimulation}
\label{small}
\end{center}
\end{figure}
...
The Kripke contraction is thus $(\{w\},\emptyset,V)$ such that $V(w,t)=\mathit{true}$. It is shown \textsc{Figure}\ \ref{small}.
\smallest
...
\paragraph{Countable number of bisimulations} Then, we show that between two finite Kripke frames, there exists only a countable number of bisimulation. Indeed, a bisimulation is included in the set $\mathcal{W}^\M\times\mathcal{W}^{\M'}$, which is countable.

\paragraph{Union of bisimulations} We can now deduce that any non-empty union of bisimulations is a bisimulation recursively on the number of bisimulations in the collection. Since this number is countable, and because the union of two bisimulations is one too, then we have that the union of the whole collection is a bisimulation as well.

\paragraph{Maximal bisimulation} Let $Z_m = \underset{X \mathit{bisimulation}}{\bigcup} X$. The union is countable because there only is a countable number of bisimulations. There is always at least one bisimulation between two Kripke models : $\emptyset$. We can thus apply the previous result and deduce that $Z_m$ is a bisimulation. Moreover, for each bisimulation $X$, by definition of $Z_m$, we have $X\subseteq Z_m$. Thus, $Z_m$ is a maximal bisimulation.
...
The formula $p\rightarrow\Box\Diamond p$ is true in \textbf{S5} because there is the Reflexivity Axiom of \textbf{T} : $p\rightarrow\Diamond p$ and the \textbf{S5} axiom : $\Diamond p\rightarrow\Box\Diamond p$.
